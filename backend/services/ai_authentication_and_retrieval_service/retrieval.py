...
#
# **下面两个值是"未配置时的安全默认"，实际运行值由配置层的
# ``RECALL_SIMILARITY_THRESHOLD`` 经 ``AiQaService`` 注入**（见第 14 章 #8）。
#
# 关于 #8 的确认值 0.8：实测该模型（text-embedding-v4）+ COSINE 下，
# 正确命中的相似度落在 **0.7383 ~ 0.7913**，无关提问的最高分是 0.4129。
# 也就是说 0.8 会把**全部真实命中**都判为不相关 —— 包括本该能命中的那条。
# 因此代码默认取实测校准值 0.5（恰好在 0.4129 与 0.7383 之间），
# `.env` 里也按此配置；要改回 0.8 只需改 `.env` 一行，不必动代码。
